...
// Mocha re-runs beforeEach + test + afterEach for each retry (browser
// session survives across retries — only Suite.afterAll tears it down).
// Each retry pauses again so the agent can propose another fix; the cap
// prevents infinite loops when the fix keeps missing.
const MAX_RETRIES = 5;
